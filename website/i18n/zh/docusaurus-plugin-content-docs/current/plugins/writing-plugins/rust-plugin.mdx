...
以下是 Farm v2 `Plugin` trait 中可用的所有 hooks。详细信息请参阅 [Rust Plugin API](/zh/docs/api/rust-plugin-api)。

**配置 Hooks：**
- `config` — 在编译前修改已解析的配置
- `plugin_cache_loaded` — 从磁盘加载插件缓存时调用
...
- `transform` — 转换模块内容
- `parse` — 将模块解析为 AST
- `process_module` — 解析后处理模块
- `analyze_deps` — 分析模块依赖
- `finalize_module` — 冻结前最终化模块
- `freeze_module` — 模块冻结时调用 (v2)
- `module_graph_build_end` — 模块图构建完成时调用 (v2)
- `build_end` — 构建完成时调用

**生成 Hooks：**
- `generate_start` — 资源生成开始时调用
- `optimize_module_graph` — 打包前优化模块图
- `freeze_module_graph_meta` — 冻结模块图元数据 (v2)
- `analyze_module_graph` — 分析模块图用于打包
- `partial_bundling` — 自定义局部打包逻辑
- `process_resource_pots` — 打包后处理 resource pots (v2)
- `render_start` — 渲染开始时调用
- `render_resource_pot` — 渲染单个 resource pot
- `process_rendered_resource_pot` — 渲染后处理 resource pot (v2)
- `augment_resource_pot_hash` — 增强 resource pot 的哈希值 (v2)
- `optimize_resource_pot` — 优化渲染后的 resource pot
- `generate_resources` — 从 resource pots 生成最终资源
- `process_generated_resources` — 生成后处理资源 (v2)
- `handle_entry_resource` — 处理入口资源 (v2)
- `finalize_resources` — 在输出前最终化所有资源
- `generate_end` — 生成完成时调用

**生命周期 Hooks：**
- `finish` — 编译完成时调用
- `write_plugin_cache` — 将插件缓存写入磁盘时调用

**HMR Hooks：**
- `update_modules` — HMR 期间确定需要更新哪些模块时调用
- `module_graph_updated` — HMR 期间模块图更新后调用 (v2)
- `update_finished` — HMR 更新完成时调用 (v2)
- `handle_persistent_cached_module` — 从持久缓存加载模块时调用 (v2)
